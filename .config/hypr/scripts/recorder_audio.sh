...
if pgrep -x "wf-recorder" > /dev/null; then
    ...
    notify-send -i "$ICON_PATH/recording-stop.png" -h string:wf-recorder:record -t 2500 "Finished Audio Recording" "Saved at $DIRECTORY"
...
if [ -n "$SELECTED_MONITOR" ]; then
    ...
    wf-recorder -aalsa_output.pci-0000_00_1f.3.analog-stereo.monitor -o "$MONITOR_NAME" --bframes max_b_frames -f "$DIRECTORY/$dateTime.mp4" &
